// NuRi's Apple Store Pickup Checker Engine (Vanilla JS)
(function (window) {
    'use strict';
    ...
    const STORE_SEEDS = ['R692', 'R747']; // 가로수길, 여의도 거점 (searchNearby로 서울/경기 전 매장 수집)
            ...
            if (!document.getElementById('watchButton')) {
                controls.innerHTML = `
                    <div class="form-check form-check-inline mb-0 d-inline-flex align-items-center gap-1" style="font-size: 0.72rem;">
                        ...
                        <label class="form-check-label text-secondary" for="audiocheck"><i class="bi bi-volume-up"></i> 소리 알림</label>
                    </div>
    ...
    function startWatch() {
        ...
        if (btn) {
            ...
            btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1" style="width:0.65rem;height:0.65rem;" role="status"></span> 감시 중';
        }
        initAudio();
        runWatchLoop();
    }
    ...
    function checkStockData(data) {
        ...
        items.forEach(item => {
            ...
            if (info && (info.availability?.contract === 'true' || info.availability?.unlocked === 'true')) {
                const shopName = shopList[shopCode] || shopCode;
                const modelName = modelList[modelCode] || modelCode;
                const log = `[${timeText}] [${shopName} ${modelName}] 재고 있음!\n`;
    ...
    function drawTable(data) {
        ...
        if (timeEl) {
            ...
            timeEl.innerHTML = `${debugBadge}<span>최근 갱신: ${formatTimestamp(date)}</span>`;
        }
        ...
        // Header
        let headHtml = '<tr><th class="model-header">종류 / 모델</th>';
        ...
        for (const modelCode of Object.keys(modelList)) {
            const internalModel = modelCode.replace('/', '_');
            bodyHtml += `<tr><th class="model-name" data-modelcode="${modelCode}"><a href="${buyUrl}" target="_blank" rel="noopener" class="text-reset text-decoration-none" title="Apple Store로 이동">${modelList[modelCode]}</a></th>`;
    ...
    async function fetchMockData() {
        const response = await fetch('test_data/mock_stock_kr.json?_t=' + Date.now(), { cache: 'no-store' });
        if (!response.ok) {
            throw new Error(`목업 파일(test_data/mock_stock_kr.json)을 불러오지 못했습니다. (HTTP ${response.status})`);
        }
        return await response.json();
    }
        ...
        try {
            if (isDebugMode) {
                ...
                if (Object.keys(stockData.stores).length === 0) {
                    if (isLocal) {
                        console.warn('Live API failed. Automatically falling back to test_data/mock_stock_kr.json (Local debug)');
                        const mockRes = await fetchMockData();
                        const agg = aggregateData(mockRes);
                        Object.assign(stockData.stores, agg.stores);
                        stockData.isDebug = true;
                    } else {
                        throw new Error('재고 데이터를 가져오지 못했습니다. 잠시 후 다시 시도하세요.');
                    }
                }
            }
            ...
            const timeEl = document.getElementById('time');
            if (timeEl) {
                timeEl.textContent = `오류: ${e.message}`;
            }
        } finally {
    ...
    window.stockEngine = {
        load: function () {
            const timeEl = document.getElementById('time');
            if (timeEl) timeEl.textContent = '재고 정보를 불러오는 중...';
            clearTable();
            return getStoreData();
        },
        refresh: function () {
