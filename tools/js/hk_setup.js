// Setup and Tab Routing for Stock Checker (HK)
(function (window, document) {
    ...
    async function switchModel(type) {
        ...
        if (timeEl) timeEl.textContent = 'Loading model data...';

        try {
            await loadScript(`js/hk_${type}.js`);
            if (window.stockEngine && typeof window.stockEngine.load === 'function') {
                window.stockEngine.load();
            }
        } catch (err) {
            console.error(err);
            if (timeEl) timeEl.textContent = `Error: Failed to load model data. (${type})`;
        }
    }
