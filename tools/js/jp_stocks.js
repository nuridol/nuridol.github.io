// NuRi's Apple Store Pickup Checker Engine (Vanilla JS - JP)
(function (window) {
    'use strict';
    ...
    const STORE_SEEDS = ['R079', 'R091', 'R048']; // Ginza, Shinsaibashi, Fukuoka Tenjin
            ...
            if (!document.getElementById('watchButton')) {
                controls.innerHTML = `
                    <div class="form-check form-check-inline mb-0 d-inline-flex align-items-center gap-1" style="font-size: 0.72rem;">
                        ...
                        <label class="form-check-label text-secondary" for="audiocheck"><i class="bi bi-volume-up"></i> 音声通知</label>
                    </div>
    ...
    function startWatch() {
        ...
        if (btn) {
            ...
            btn.innerHTML = '<span class="spinner-border spinner-border-sm me-1" style="width:0.65rem;height:0.65rem;" role="status"></span> 監視中';
        }
        initAudio();
        runWatchLoop();
    }
    ...
    function checkStockData(data) {
        ...
        items.forEach(item => {
            ...
            if (info && (info.availability?.contract === 'true' || info.availability?.unlocked === 'true')) {
                const shopName = shopList[shopCode] || shopCode;
                const modelName = modelList[modelCode] || modelCode;
                const log = `[${timeText}] [${shopName} ${modelName}] 在庫あり!\n`;
    ...
    function drawTable(data) {
        ...
        if (timeEl) {
            ...
            timeEl.innerHTML = `${debugBadge}<span>最終更新: ${formatTimestamp(date)}</span>`;
        }
        ...
        // Header
        let headHtml = '<tr><th class="model-header">種類 / モデル</th>';
        ...
        for (const modelCode of Object.keys(modelList)) {
            const internalModel = modelCode.replace('/', '_');
            bodyHtml += `<tr><th class="model-name" data-modelcode="${modelCode}"><a href="${buyUrl}" target="_blank" rel="noopener" class="text-reset text-decoration-none" title="Apple Storeへ">${modelList[modelCode]}</a></th>`;
    ...
    async function fetchMockData() {
        const response = await fetch('test_data/mock_stock_jp.json?_t=' + Date.now(), { cache: 'no-store' });
        if (!response.ok) {
            throw new Error(`モックファイル(test_data/mock_stock_jp.json)の取得に失敗しました。(HTTP ${response.status})`);
        }
        return await response.json();
    }
        ...
        try {
            if (isDebugMode) {
                ...
                if (Object.keys(stockData.stores).length === 0) {
                    if (isLocal) {
                        console.warn('Live API failed. Automatically falling back to test_data/mock_stock_jp.json (Local debug)');
                        const mockRes = await fetchMockData();
                        const agg = aggregateData(mockRes);
                        Object.assign(stockData.stores, agg.stores);
                        stockData.isDebug = true;
                    } else {
                        throw new Error('在庫データを取得できませんでした。しばらく経ってから再度お試しください。');
                    }
                }
            }
            ...
            const timeEl = document.getElementById('time');
            if (timeEl) {
                timeEl.textContent = `エラー: ${e.message}`;
            }
        } finally {
    ...
    window.stockEngine = {
        load: function () {
            const timeEl = document.getElementById('time');
            if (timeEl) timeEl.textContent = '在庫情報を取得中...';
            clearTable();
            return getStoreData();
        },
        refresh: function () {
